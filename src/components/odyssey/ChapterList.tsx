    ...
    <nav className="flex flex-col h-full" aria-label="Chapter navigation">
      <div className="px-4 py-3 border-b">
        <h2 className="text-xs uppercase tracking-widest text-muted-foreground flex items-center gap-2">
          <BookOpen className="h-3.5 w-3.5" /> {language === "es" ? "Contenido" : "Contents"}
        </h2>
      </div>
      <ScrollArea className="flex-1 odyssey-scroll">
        <ul className="py-1">
          {CHAPTER_MANIFEST.map((c) => {
            ...
            const cacheKey = `${language}:${c.slug}`;
            const loaded = chapters.get(cacheKey);
            const progress = scrollProgress[c.slug] ?? 0;
            const label = language === "es" ? c.labelEs : c.label;
            return (
              <li key={c.slug}>
                <button
                  className={cn(
                  ...
                  <span className="flex-1 min-w-0">
                    ...
                    {loaded && (
                      <span className="block text-[10px] text-muted-foreground">
                        {loaded.subtitle || `${loaded.wordCount.toLocaleString()} ${language === "es" ? "palabras" : "words"}`}
                      </span>
                    )}
                  </span>
                  {progress > 0 && (
                    <span
                      ...
                      aria-label={`${Math.round(progress * 100)}% ${language === "es" ? "leído" : "read"}`}
                    >
